  ...
  if (current_block) {
    ...
    if (step_events_completed >= step_event_count) {
      ...
      if (cutter.cutter_mode == CUTTER_MODE_DYNAMIC
        ...
        //SERIAL_ECHOLNPAIR("stepper.cpp", current_block->laser.power); //107011
    ...
    #if ENABLED(LASER_FEATURE)
      if (cutter.cutter_mode == CUTTER_MODE_DYNAMIC) cutter.apply_power(0);  // No movement in dynamic mode so turn Laser off
      ...
      if ( ENABLED(HAS_L64XX)       // Always set direction for L64xx (Also enables the chips)
        ...
        if (cutter.cutter_mode == CUTTER_MODE_CONTINUOUS) {           // Planner controls the laser
          ...
          if (stat.isEnabled) {
            ...
            #else
              //SERIAL_ECHOLNPAIR("stepper.cpp 2224 =", current_block->laser.power);
