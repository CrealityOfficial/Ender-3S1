...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=POWER_LOSS_ZRAISE*/, const bool raised/*=false*/) {
  ...
  #endif
  #if ENABLED(CREALITY_ENDER3_2021)	 //rock_20211016
    if(info.target_temperature[0] != thermalManager.temp_hotend[0].target || info.target_temperature_bed != thermalManager.temp_bed.target)
    {
      info.target_temperature[0] = thermalManager.temp_hotend[0].target;
      info.target_temperature_bed = thermalManager.temp_bed.target;
      #ifdef EEPROM_PLR
      SERIAL_ECHOLNPAIR("info.target_temperature[0]=: ", info.target_temperature[0]);
      SERIAL_ECHO_MSG("\r\n");
      SERIAL_ECHOLNPAIR("info.target_temperature_bed=: ", info.target_temperature_bed);
      SERIAL_ECHO_MSG("\r\n");
  ...
  if (force
    ...
    info.feedrate = uint16_t(MMS_TO_MMM(feedrate_mm_s));  //rock_20211016
    info.zraise = zraise;
    ...
    info.sd_printing_flag = true;    //rock_20210728
...
/**
 * Resume the saved print job 恢复保存的打印作业
 */
#if ENABLED(CREALITY_ENDER3_2021)
void PrintJobRecovery::resume() {
  ...
  // Restore cold extrusion permission  恢复冷挤压许可
  ...
  // Interpret the saved Z according to flags 根据标记解释保存的Z
  ...
  gcode.process_subcommands_now_P(PSTR("G92.9E0")); // Reset E to 0  rock_20210929
  ...
  //Additional purge on resume if configured
    ...
    sprintf_P(cmd, PSTR("G1 E%d F500"), (POWER_LOSS_PURGE_LEN) + (POWER_LOSS_RETRACT_LEN));
    gcode.process_subcommands_now(cmd);
  #endif
  ...
  // Move back down to the saved Z for printing  向下移动到保存的Z用于打印
  ...
  //info.current_position.e-=5;  //rock_20210927
  ...
  // // if(info.sdpos>=96*3)
  // //   info.sdpos-=96*3; //rock_20210819
  // if(info.sdpos>=96*2)
  //  info.sdpos-=96*2; //rock_20210819;

  sprintf_P(cmd, PSTR("M24S%ldT%ld"), resume_sdpos, info.print_job_elapsed);
  gcode.process_subcommands_now(cmd);
  

  // SERIAL_ECHOLNPAIR("Count E222:", stepper.position(E_AXIS));
  // SERIAL_ECHOLNPAIR("info.current_position.e111:", info.current_position.e);
  info.current_position.e-=2;
  // SERIAL_ECHOLNPAIR("info.current_position.e2222:", info.current_position.e);

  sprintf_P(cmd, PSTR("G92.9E%s"), dtostrf(info.current_position.e, 1, 3, str_1));
  gcode.process_subcommands_now(cmd);

  // SERIAL_ECHOLNPAIR("info.current_position.e3333:", info.current_position.e);
  // SERIAL_ECHOLNPAIR("Count 33333:", stepper.position(E_AXIS));
...
void PrintJobRecovery::resume()
 {
  ...
  feedrate_percentage=info.feedrate_percentage;  //rock_20211016
  #if Z_HOME_TO_MAX
    float z_now = z_raised;
  ...
  set_all_homed();
    ...
    // #if HOME_XY_ONLY  //在自动调平打开前恢复z轴坐标，自动调平后后不恢复 rock_20211201
    //   // The physical Z was adjusted at power-off so undo the M420S1 correction to Z with G92.9.
    //   sprintf_P(cmd, PSTR("G92.9Z%s"), dtostrf(z_now, 1, 1, str_1));
    //   gcode.process_subcommands_now(cmd);
    // #endif
  ...
  sprintf_P(cmd, PSTR("G1Z%sF600"), dtostrf(z_print, 1, 3, str_1)); //rock_20211206
  gcode.process_subcommands_now(cmd);   
  ...
  if(HMI_flag.disallow_recovery_flag)
  {
    ...
    sprintf_P(cmd, PSTR("M24S%ldT%ld"), resume_sdpos, info.print_job_elapsed);
    gcode.process_subcommands_now(cmd); 
    HMI_flag.disallow_recovery_flag=false;//rock_20211009  
